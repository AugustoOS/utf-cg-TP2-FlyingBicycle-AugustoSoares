const Rocks = {
    ...
    async init() {
        ...
        try {
            this.texture = await GLPanel.loadTextureFromUrl('../assets/models/rocks/GreyRockTexture.png');
        } catch {
            const gl = GLPanel.state.gl;
            this.texture = gl.createTexture();
            gl.bindTexture(gl.TEXTURE_2D, this.texture);
            gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, 1, 1, 0, gl.RGBA, gl.UNSIGNED_BYTE,
                new Uint8Array([130, 120, 110, 255]));
    ...
    _rand(cx, cz, i) {
        return Math.abs(Math.sin(cx * 337.9 + cz * 167.3 + i * 251.7) * 43758.5453) % 1;
    },
